...
export class PostService {
  ...
  createPost(data: IPost): Observable<IPost> {
    return this.http.post<IPost>(
      `${this.apiUrl}${API_URLS.POSTS.ADD_POST}`,
      data
    );
  }
  ...
  getSinglePost(postid: string) {
    return this.http.get<IResponse<IPost[]>>(
      `${this.apiUrl}${API_URLS.POSTS.GET_SINGLE_POST(postid)}`
    );
  }

  likePost(postid: string) {
    return this.http.post<any>(`${this.apiUrl}${API_URLS.POSTS.LIKE_POST}`, {
      postid,
    });
  }
  ...
  unlikePost(postid: string) {
    ...
    return this.http.delete<IResponse<IPostLike>>(
      `${this.apiUrl}${API_URLS.POSTS.UNLIKE_POST}`,
      options
    );
  }

  savePost(postid: string) {
    return this.http.put<any>(`${this.apiUrl}${API_URLS.POSTS.SAVE_POST}`, {
      postid,
    });
  }
  unsavePost(postid: string) {
    return this.http.delete<any>(
      `${this.apiUrl}${API_URLS.POSTS.UNSAVE_POST(postid)}`
    );
  }

  getSavedPost() {
    return this.http.get<IResponse<IPost[]>>(
      `${this.apiUrl}${API_URLS.POSTS.GET_SAVED_POST}`
    );
  }
